...
void AddBody(NetParameter* net) {
  ...
  // 27 x 27
  AddMixedLayer("mixed2a", "pool1", 32, 48, 64, 8, 16, 16, net);

  // 27 x 27.
  AddPoolingLayer("pool2", "mixed2a", 3, 2, 0, PoolingParameter::MAX,
                  net->add_layers());

  // 13 x 13
  AddConvolutionLayer("conv3", "pool2", 96, 3, 0, 1, net->add_layers());
  AddReluLayer("relu2", "conv3", net->add_layers());

  // 6 x 6.
  AddPoolingLayer("pool3", "conv3", 6, 1, 0, PoolingParameter::AVE, net->add_layers());

  // Dropout.
  AddDropoutLayer("drop", "pool3", 0.4, net->add_layers());
