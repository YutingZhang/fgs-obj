...
void AddBody(NetParameter* net) {
  ...
  AddMixedLayer("mixed2b", "mixed2a", 64, 96, 128, 16, 32, 32, net);

  // Dimensionality reduction - 13 x 13.
  AddConvolutionLayer("conv2", "mixed2b", 256, 3, 0, 2, net->add_layers());

  // Inception layers - 13 x 13.
  AddMixedLayer("mixed3a", "conv2", 128, 128, 192, 32, 48, 48, net);
  AddMixedLayer("mixed3b", "mixed3a", 128, 128, 192, 32, 48, 48, net);

  // Dimensionality reduction - 7 x 7.
  AddConvolutionLayer("conv3", "mixed3b", 480, 3, 0, 2, net->add_layers());

  // Inception layers - 7 x 7.
  AddMixedLayer("mixed4a", "conv3", 256, 160, 320, 32, 128, 128, net);
  AddMixedLayer("mixed4b", "mixed4a", 384, 192, 384, 48, 128, 128, net);

  // Average Pooling
  AddPoolingLayer("pool2", "mixed4b", 6, 1, 0, PoolingParameter::AVE, net->add_layers());
